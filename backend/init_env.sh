...
if ! command -v mysql >/dev/null 2>&1; then
  echo "警告: 未找到 mysql 客户端，跳过连接检测与自动建库。请安装 MySQL 客户端后手工建库并配置 backend/.env；若本地有 docs/OPERATIONS.md 可参考。" >&2
  exit 0
fi
...
PY
  ...
  echo "提示: 未设置 INIT_MYSQL_ADMIN_PASSWORD，跳过自动建库。请手工创建数据库与用户，密码须与 .env 中 DB_PASSWORD 一致；若本地有 docs/OPERATIONS.md 可参考。"
fi
